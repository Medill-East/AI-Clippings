 ...
 * via WeChat Web (wx.qq.com) and merge into the local JSONL index.
 *
 * Usage:
...
import { launchBrowser, ensureLoggedIn } from "./lib/session.js";
import { openFileHelperChat, scrollAndCollect } from "./lib/chat.js";
import {
  readJsonlines,
  writeJsonlines,
  mergeRecords,
...
Options:
  --since <ISO8601>     Start time (inclusive), e.g. 2026-03-22T15:00:00+08:00
  --until <ISO8601>     End time (inclusive), e.g. 2026-03-22T23:59:59+08:00
...
async function main() {
  ...
  const profileDir = path.join(skillRoot, "local/browser-profile");
  ...
  console.log("WeChat FileHelper Web Ingest — Scan");
  ...
  try {
    ...
    console.log("Opening WeChat Web...");
    await ensureLoggedIn(page);

    console.log("Navigating to 文件传输助手...");
    await openFileHelperChat(page);

    console.log("Scanning messages...");
    newRecords = await scrollAndCollect(page, opts.since, opts.until, opts.maxScrolls, opts.debug);
  } finally {
    await browser?.close();
  }
