...
本机若存在 `local/ui-layout.json` 且 `mode` 为 `docked_articles`，原来的 `scan` / `collect` 命令会自动使用校准布局。启动前先在文件传输助手打开第一篇文章，保持聊天与阅读栏并排；不要在扫描中改变窗口大小。

- `windowWidth` / `windowHeight` 为展开后的窗口尺寸（逻辑点），`chatWidth` 为左侧聊天区域宽度。本次用户截图校准为 1470 / 923 / 735；标签关闭按钮通过当前截图中的图像匹配定位，不再使用固定关闭坐标。这是本机校准值，不是所有微信窗口的通用比例。
- 只扫描、复制和滚动左侧聊天区域；文章与内嵌视频逐篇复制链接后保持阅读栏展开，扫描结束或抛错后再统一点击文章标签自身的关闭按钮；不使用会关闭微信主窗口的 Cmd+W。
...
- 裸 `HH:mm` 按历史消息解析，不会归到扫描时刻之后的未来；例如 9/14 凌晨看到的 `21:06` 属于 9/13。
- 可见聊天时间适用于它后面的整组消息，不以距离远近丢失时间；位于下一组时间标记之前、尚缺前置时间的卡片会暂缓，向上滚动补足时间后再处理。
- 没有任何可见时间上下文时仍保留原有占位策略，并标明来源；这不是伪造一个确切消息时间。明确早于 since 的卡片和输入区按钮不会进入提链。
- 一轮收集以 URL 唯一计数，不因同一卡片再次出现时的占位时间不同而重复入库。
- 视频分享菜单为悬停菜单：按右侧视频区域定位，截图后恢复悬停再复制，并保留 `/sph/` 链接供原后台管线使用。
