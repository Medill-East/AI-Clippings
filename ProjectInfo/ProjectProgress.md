...
- 视频号“已取得 `/sph/` 后”的后台全链路已完成单卡真实验收：自动关闭 viewer 后解析、下载、转写、总结、写回均不等待视频播完。
- 今天的真实 FileHelper 画面实际有 2 张视频号卡片，但采集候选只生成 1 条：上方“项目发展规划／大学生创新创业大赛”已写入笔记；下方只露出部分的“OCR+翻译／粘贴神器”未取得链接、未建任务、未写笔记。当前缺口在前端候选发现，不在后台处理。
...
- 视频号第一版产品契约已在“成功提链的单卡”上闭环：原始链接 + 自有高质量摘要 + 关键要点；不默认保留视频或逐字稿。
- 当前代码具备时间范围批处理、逐条持久化、失败留痕和幂等重试；但尚未保证一屏内部分可见的视频卡片也会进入候选队列。今天两卡只处理一条，不能称为真实批次闭环。

## 下一步

- 先修复并验证“部分可见视频卡片漏候选”：同一屏两张卡必须产生两个不同候选，必要时滚动使卡片完整可见后再提链；两条都应分别留下索引、task 与笔记证据。
- 修复后再用原 `collect` 命令重跑今天这两张卡，确认第二条“OCR+翻译／粘贴神器”被补抓；不需要手工播放视频。
...
- 当前即时阻塞是第二张视频卡尚无 `/sph/` 链接；后台处理器无法选择一个从未进入索引的项目。
- 元宝解析接口是未公开 Web 接口，存在漂移风险；登录态也可能过期，但两者已有不同错误码。
- Codex 摘要会把临时逐字稿发送给用户已登录的 Codex 服务；元宝只看到公开分享链接，不会收到逐字稿。
- 真实证据目前只有一条 5 分 28 秒视频。今天两卡画面的第二张因部分可见而漏采，说明多卡候选发现尚未通过真实验收；后台仍故意顺序处理以保证失败归因和本机资源稳定。
